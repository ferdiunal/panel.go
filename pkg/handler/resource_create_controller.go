...
func HandleResourceCreate(h *FieldHandler, c *context.Context) error {
	...
	resourceCtx := ensureResourceContext(c, h.Resource, nil, fields.ContextCreate)
	...
	for _, element := range elements {
		...
		// Stack field çocuklarını form bağlamında çözümle.
		// Bu sayede create formunda stack içindeki alanlar da data/options ile gelir.
		h.resolveStackFieldChildren(resourceCtx, nil, serialized)
