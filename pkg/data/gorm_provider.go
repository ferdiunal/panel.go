...
// getClient - Private metod, sadece internal kullanım için
// Handler'lar bu metoda erişemez, sadece Provider içinde kullanılır
func (p *GormDataProvider) getClient() *gorm.DB {
	return p.DB
}

// GetClient - Public metod, Card.Resolve() ve Action.Execute() için
//
...
// # Döndürür
//
// - interface{}: GORM DB instance (*gorm.DB olarak type assertion yapılmalı)
//
// # Örnek Kullanım
//
// ```go
// db, ok := provider.GetClient().(*gorm.DB)
//
//	if !ok {
...
// ```
func (p *GormDataProvider) GetClient() interface{} {
	return p.DB
}

// QueryTable - Dinamik tablo query'leri için (MorphTo, HasOne, BelongsTo için)
//
// Bu metod, dinamik tablo adı ile query çalıştırmak için kullanılır.
// MorphTo, HasOne, BelongsTo gibi field'lar için gereklidir.
//
// # Parametreler
//
// - ctx: İstek bağlamı
...
//	    "status": "active",
//	})
func (p *GormDataProvider) QueryTable(ctx *context.Context, table string, conditions map[string]interface{}) ([]map[string]interface{}, error) {
	db := p.getClient()
	...
	if ctx == nil {
		...
		query = db.WithContext(ctx.Context()).Table(table)
	}

	for key, value := range conditions {
		safeKey := SanitizeColumnName(key)
...
// QueryRelationship - Relationship query'leri için (display field çekmek için)
//
// Bu metod, relationship field'larının display değerini çekmek için kullanılır.
// Örneğin, BelongsTo field'ında foreign key ile ilişkili kaydın adını çekmek için.
//
// # Parametreler
//
// - ctx: İstek bağlamı
...
func (p *GormDataProvider) QueryRelationship(ctx *context.Context, relationshipType string, foreignKey string, foreignValue interface{}, displayField string) (interface{}, error) {
	db := p.getClient()
	table := strings.ToLower(relationshipType)
	safeForeignKey := SanitizeColumnName(foreignKey)
	safeDisplayField := SanitizeColumnName(displayField)
	...
	err := query.Where(safeForeignKey+" = ?", foreignValue).
		Select(safeDisplayField).
		First(&result).Error
	...
	return result[displayField], nil
}

// BeginTx - Yeni bir transaction başlatır
//
// Bu metod, yeni bir transaction başlatır ve transaction içinde çalışan
// yeni bir Provider döndürür. Transaction içinde yapılan tüm işlemler
// Commit() veya Rollback() çağrılana kadar bekletilir.
//
// # Parametreler
//
// - ctx: İstek bağlamı
...
// # Döndürür
//
// - DataProvider: Transaction içinde çalışan yeni Provider
// - error: Hata durumunda hata mesajı
//
// # Örnek Kullanım
//
...
func (p *GormDataProvider) BeginTx(ctx *context.Context) (DataProvider, error) {
	db := p.getClient()
	...
	if ctx == nil {
		...
		tx = db.WithContext(ctx.Context()).Begin()
	}
...
// Commit - Aktif transaction'ı commit eder
//
// Bu metod, BeginTx() ile başlatılan transaction'ı commit eder.
// Transaction içinde yapılan tüm değişiklikler veritabanına yazılır.
//
// # Döndürür
//
// - error: Hata durumunda hata mesajı
...
// # Örnek Kullanım
//
//	err := txProvider.Commit()
func (p *GormDataProvider) Commit() error {
	db := p.getClient()
	return db.Commit().Error
}

// Rollback - Aktif transaction'ı rollback eder
//
// Bu metod, BeginTx() ile başlatılan transaction'ı rollback eder.
// Transaction içinde yapılan tüm değişiklikler geri alınır.
//
// # Döndürür
//
// - error: Hata durumunda hata mesajı
...
// # Örnek Kullanım
//
//	err := txProvider.Rollback()
func (p *GormDataProvider) Rollback() error {
	db := p.getClient()
	return db.Rollback().Error
}

// Raw - Raw SQL query çalıştırır ve sonuçları döndürür (SELECT için)
//
// Bu metod, raw SQL query çalıştırmak için kullanılır.
// Özel durumlar için kullanılmalıdır, normal işlemler için Provider metodları tercih edilmelidir.
//
// # Parametreler
//
// - ctx: İstek bağlamı
// - sql: SQL query
// - args: Query parametreleri
//
// # Döndürür
//
// - []map[string]interface{}: Sorgu sonuçları
...
func (p *GormDataProvider) Raw(ctx *context.Context, sql string, args ...interface{}) ([]map[string]interface{}, error) {
	db := p.getClient()
	var results []map[string]interface{}
	...
	if ctx == nil {
		...
		err = db.WithContext(ctx.Context()).Raw(sql, args...).Scan(&results).Error
	}

	return results, err
}

// Exec - Raw SQL query çalıştırır (INSERT, UPDATE, DELETE için)
//
// Bu metod, raw SQL query çalıştırmak için kullanılır.
// Özel durumlar için kullanılmalıdır, normal işlemler için Provider metodları tercih edilmelidir.
//
// # Parametreler
//
// - ctx: İstek bağlamı (nil olabilir)
// - sql: SQL query
// - args: Query parametreleri
//
// # Döndürür
...
func (p *GormDataProvider) Exec(ctx *context.Context, sql string, args ...interface{}) error {
	db := p.getClient()
	...
	return db.WithContext(ctx.Context()).Exec(sql, args...).Error
}
