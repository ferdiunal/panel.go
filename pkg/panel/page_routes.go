...
func (p *Panel) handlePageDetail(c *context.Context) error {
	...
	for _, f := range pg.Fields() {
		...
		/// Ayarlar sayfası için özel işleme
		/// Ayarlar sayfasında, alanların değerleri dinamik olarak enjekte edilir
		if pg.Slug() == "settings" {
			if key, ok := serialized["key"].(string); ok {
				/// Önce dinamik ayar değerlerini kontrol et
				if val, exists := p.Config.SettingsValues.Values[key]; exists {
					serialized["data"] = val
				} else {
					/// Dinamik değer yoksa, struct alanlarından varsayılan değerleri al
					switch key {
					case "site_name":
